...
$null = New-Module starship {
    ...
    function Invoke-Native {
        param($Executable, $Arguments, [switch]$NoWait)
    ...
    # background: PSReadLine's InvokePrompt is not thread-safe off the
    # pipeline thread, and PowerShell.OnIdle stops firing for the rest of the
    # session once a child process is spawned (verified directly on pwsh
