...
fn paint_line(
    ...
    if !chunks.is_empty() {
        // Leftover width splits evenly; a line already full falls back to natural width.
        let fill_width = available_width.and_then(|TerminalWidth(available)| {
            (available > used_width).then(|| (available - used_width) / chunks.len())
        });

        for (chunk, fill) in chunks {
